...
string createArrowDir(double x1, double y1, double x2, double y2, std::string & color, Point2D& midPoint) {
    string gs("");
    ...
    midPoint.x = (c1.x + c4.x) / 2;
    midPoint.y = (c1.y + c4.y) / 2;
...
std::string computeGraph1() {
    ...
    std::string col("#00FF00");
    Point2D m;
    s+=createArrowDir(p1.x, p1.y, p2.x, p2.y, col, m);

    s += "<line x1=\"" + f(m.x) + "\" y1=\"" + f(m.y) + "\" x2=\"" + f(p2.x) + "\" y2=\"" + f(p2.y) + "\" style=\"stroke:rgb(255,0,0);stroke-width:1\"/>";
...
std::string computeGraph() {
    ...
    Point2D midPoint;
    s += createArrowDir(p4.x,p4.y,p.x, p.y, col,midPoint);

    //s += "\n<circle cx=\"" + fd(p.x) + "\" cy=\"" + fd(p.y) + "\" r=\"0.5\" stroke=\"red\" stroke-width=\"0.5\" fill=\"pink\" />";
    ...
    s += "\" stroke=\"#0000FF\" stroke-width=\"0.5\" fill=\"none\"/>";
