...
ImageProcessing::ImageProcessing(const char *_inImgName,
                                 const char *_outImgName, int *_height,
                                 int *_width, int *_bitDepth,
                                 unsigned char *_header,
                                 unsigned char *_colorTable,
                                 unsigned char *_inBuf, unsigned char *_outBuf
...
void ImageProcessing ::copyImgData(unsigned char *_srcBuf,
                                   unsigned char *_destBuf, int bufSize) {
  for (int i = 0; i < bufSize; i++) {
    _destBuf[i] = _srcBuf[i];
  }
}

void ImageProcessing::binarizeImage(unsigned char *_inImgData,
...
void ImageProcessing::brigthnessUp(unsigned char *_inputImgData,
                                   unsigned char *_outImgData, int imgSize,
                                   int brightness) {
  for (int i = 0; i < imgSize; i++) {
...
void ImageProcessing::brigthnessDown(unsigned char *_inputImgData,
                                     unsigned char *_outImgData, int imgSize,
                                     int darkness) {
  for (int i = 0; i < imgSize; i++) {
...
void ImageProcessing::computeHistogram(unsigned char *_imgData, int imgRows,
                                       int imgCols, float hist[]) {
  FILE *fptr;
  fptr = fopen("image_hist.txt", "wb");
...
void ImageProcessing::computeHistogram2(unsigned char *_imgData, int imgRows,
                                        int imgCols, float hist[],
                                        const char *histFile) {
  FILE *fptr;
...
void ImageProcessing::equalizeHistogram(unsigned char *_inputImgData,
...
void ImageProcessing::getImageNegative(unsigned char *_inImgData,
                                       unsigned char *_outImgData, int imgWidth,
                                       int imgHeight) {
...
void ImageProcessing::Convolve2D(int imgRows, int imgCols, struct Mask *myMask,
    ...
    for (j = 0; j < imgCols; ++j) {
      ...
      for (m = 0; m < myMask->Rows; ++m)
        for (n = 0; n < myMask->Cols; ++n) {
          ...
          idx = i - m;
          jdx = j - n;
          if (idx >= 0 && jdx >= 0) im = *(input_buf + idx * imgRows + jdx);
          val += ms * im;
        }
      if (val > 255) val = 255;
      if (val < 0) val = 0;
...
void ImageProcessing::detectLine(unsigned char *_inputImgData,
                                 unsigned char *_outputImgData, int imgCols,
                                 int imgRows, const int MASK[][3]) {
  int x, y, i, j, sum;
